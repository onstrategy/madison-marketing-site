...
case "$KIND" in
  build)
    CTX="This looks like a BUILD request (non-technical contributor). Run the build workflow: derive a short page name and a one-line description, scaffold it with the generator (bun run gen:prototype, giving slug + title + description, all three non-empty), load the design-system skill, compose @madison/ui primitives on-token (never raw Tailwind or hex), run bun run check, and report what you built in plain words. Never hand them a localhost URL as the preview link — it resolves only on this machine and cannot be shared; name the page instead, and offer /submit, which returns a real Deploy Preview link. Do all the tooling yourself. See docs/prompts.md or the /build command."
    ;;
  restyle)
