...
const AssetNameSchema = z
  .string()
  .regex(/^[a-z0-9]+(?:-[a-z0-9]+)*\.(?:png|jpe?g|webp|avif|pdf)$/);
...
export const WebinarDocumentSchema = z
  .object({
...
export type WebinarDocument = z.infer<typeof WebinarDocumentSchema>;

export type WebinarSource = {
  source: string;
  value: unknown;
};

export type WebinarCollectionResult =
  | { ok: true; value: WebinarDocument[] }
...
export function buildWebinarCollection(
  sources: WebinarSource[],
): WebinarCollectionResult {
  const entries: Array<{ source: string; document: WebinarDocument }> = [];
  const errors: string[] = [];

  for (const source of sources) {
    const result = WebinarDocumentSchema.safeParse(source.value);
    if (!result.success) {
  ...
  for (const { source, document } of entries) {
    ...
    if (previousIdSource) {
      errors.push(
        `${document.id}: duplicate webinar id in ${previousIdSource} and ${source}`,
      );
    } else {
